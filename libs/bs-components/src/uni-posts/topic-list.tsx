...
import { TopicBox } from '../topic-box/topic-box';
...
export function UniPostsTopics(props: Partial<UniPostsTopicsProps>) {
  ...
  function renderRow({ index, style }: ListChildComponentProps) {
    ...
    return (
      <li style={style} className="list-group-item">
        <TopicBox item={item} onClickMenuItem={onClickRowMenuItem}/>
      </li>
    );
  }
  ...
  async function _updateOne(arg: PutUniPostsByIdApiArg) {
    ...
    try {
      ...
      if (res.data.data) {
        if (arg.body.data) { 
          const keys = Object.keys(arg.body.data);
          const changes = { attributes: {} } as Partial<UniPostsPost>;
          //@ts-ignore
          for (const key of keys) changes.attributes[key] = res.data.data.attributes[key];
          console.log('>>>>>>>>>>>>', changes);

          const idx = entityData.current.findIndex((e) => e.id === arg.id );
          if (idx >= 0) {
            const mergedAttr = { ...entityData.current[idx].attributes, ...changes.attributes }
            entityData.current.splice(idx, 1, { id: arg.id, attributes: mergedAttr });
            //entityData.current[idx].attributes = {...entityData.current[idx].attributes, ...changes.attributes};
          }
          // store
          uniPostsCacheUpdate({ id: arg.id, changes }, store.dispatch);
        }
      }
